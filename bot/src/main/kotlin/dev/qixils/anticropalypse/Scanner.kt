...
fun Long.toBytes(size: Int, big: Boolean): ByteArray {
    val result = ByteArray(size)
    for (i in 0 until size) {
        result[i] = if (big)
            (this shr (size - i - 1) * 8).toByte()
...
fun InputStream.parsePNGChunk(): PNGChunk {
    ...
    return PNGChunk(
        ctype,
        body,
        size.toBytes(4, true) + ctype + body + crc.toBytes(4, true),
    )
}
...
class Scanner {
    ...
    /**
     * Strips all trailing data from an image if possible.
     *
     * @param url the URL of the image to scan
     * @return input stream of the stripped image, or null if the image could not be downloaded
     */
    suspend fun strip(url: String): InputStream? {
        // download image
        val request = http.newCall(Request.Builder().url(url).build())
        try {
            return request.awaitWith {
                if (it.code != 200)
                    return@awaitWith null
                val stream: InputStream = it.body!!.byteStream()
                return@awaitWith strip(stream)
            }
        } catch (e: IOException) {
            return null
        } catch (e: Exception) {
     ...
     * Strips all trailing data from an image if possible.
     *
     * @param input the input stream of the image to scan
     * @return input stream of the stripped image (or the original input if the image could not be stripped)
     */
    fun strip(input: InputStream): InputStream {
        try {
            val output = ByteArrayOutputStream()
            // ensure image is PNG
            val bytes = input.readNBytes(header.size)
            if (bytes.size != header.size)
                return input
            for (i in header.indices) {
                if (bytes[i] != header[i])
            ...
            // find end of cropped PNG
            while (true) {
                val chunk = input.parsePNGChunk()
                output.write(chunk.raw)
                if (chunk.type.contentEquals(IEND))
                    break
            }
            input.closeQuietly()
